...
def render_rays(models,
    ...
    if perturb > 0:
        ...
        perturb_rand = torch.rand(z_vals.shape, device=z_vals.device)
    ...
    result = {'rgb_coarse': rgb_coarse, 'depth_coarse': depth_coarse}

    # re-sample fine points
    if N_importance > 0:
        z_vals_mid = .5 * (z_vals[:, :-1] + z_vals[:, 1:])
        ...
        xyz_fine_sampled = rays_o.unsqueeze(1) + \
            rays_d.unsqueeze(1) * z_vals.unsqueeze(2) # (N_rays, N_samples+N_importance, 3)

        # fine model inference
        if not test_time:
            rgb_fine, depth_fine, _ = \
                inference(models[1], embedding_xyz, xyz_fine_sampled, rays_d, dir_embedded, z_vals)
